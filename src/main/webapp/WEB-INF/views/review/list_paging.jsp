...
<DIV class='content_body'>
  ...
  <DIV style="text-align: right; clear: both;">  
    <form name='frm' id='frm' method='get' action='./list_paging.do'>
      <input type='hidden' name='contentsno' value='${reviewMemberVO.contentsno }'>
      <input type='hidden' name='now_page' value='1'> <%-- 최초 검색시 시작 페이지 지정 --%>
      <input type='text' name='content' id='content' value='${param.content }' style='width: 20%;'>
      <button type='submit'>검색</button>
      <c:if test="${param.content.length() > 0 }">
        <button type='button' 
                     onclick="location.href='./list_paging.do?contentsno=${reviewMemberVO.contentsno}&content='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
